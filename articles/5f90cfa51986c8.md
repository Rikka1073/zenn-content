...
`Vite`を開発環境で起動すると 5173 番ポートに接続されます。5173 番ポートの内容が`Nginx`に反映されるように設定します。
...
- 以下のコマンドで設定を反映させるために nginx を再起動します。

```
sudo systemctl restart nginx
```

- このようなエラー画面が出ていれば完了です。`http://localhost:5173`にまだ React のアプリがないのでこのような状態になっています。
...
:::message
`Vite`で作成した React のアプリは事前に作成されており`GitHub`にプッシュされている前提で進めていきます。
:::
...
- 以下のコマンドで該当のレポジトリをクローンします。今回は`aws-vite-app`というレポジトリをクローンします。
...
- 以下のコマンドで`nvm`を使用して`Node.js`の最新の LTS バージョンをインストールします。
...
- `Node.js`のバージョンを表示されていれば完了です。

```
Running Node.js v22.14.0
```

### React アプリの起動

- 以下のコマンドでクローンしたディレクトリに移動します。

```
cd aws-vite-app
```

- 以下のコマンドで`npm`をインストールします。
...
- `パブリック IPv4 アドレス`か`パブリック IPv4 DNS`にブラウザからアクセスします。`Vite`のデフォルト画面が表示されれば完了です。

![](https://storage.googleapis.com/zenn-user-upload/753cc5c4eb1d-20250325.png)

## Route 53 で取得したドメインに表示

現在は `EC２`のパブリック IP でアプリを表示させています。これを`Route 53`で取得したドメインに表示させていきます。
...
入力が完了したら`シンプルなレコードを定義`をクリックして完了です。

![](https://storage.googleapis.com/zenn-user-upload/0055c378286a-20250325.png)

- 取得したドメインにアクセスして`Vite`のデフォルト画面が表示されれば完了です。

![](https://storage.googleapis.com/zenn-user-upload/753cc5c4eb1d-20250325.png)

# 終わりに

以上が `EC2`と`Route 53`で`Vite`で React のアプリをデプロイする方法になります。
